...
func InitEtcd(etcd *clientv3.Client, ctx context.Context) {
	...
	values := []string{`{"id":90001,"risk_level":4,"seclang":"Include ruleset/coraza.conf"}`, `{"id":90002,"risk_level":4,"seclang":"Include ruleset/coreruleset/crs-setup.conf.example"}`, `{"id":90003,"risk_level":4,"seclang":"Include ruleset/coreruleset/rules/*.conf"}`, `{"id":1,"is_buildin":1,"rule_id_list":[90001,90002,90003]}`}
	// 遍历键值对
