...
func (w wafStrategyRepo) Create(ctx context.Context, strategyInfo model.Strategy) (int64, error) {
	...
	err := w.data.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Table(model.StrategyConfigTableName).Create(&strategyInfo.StrategyConfig).Error; err != nil {
			return err
		}
...
func (w wafStrategyRepo) Update(ctx context.Context, id int64, strategyInfo model.Strategy) error {
	...
	err := w.data.db.Transaction(func(tx *gorm.DB) error {
		...
		// 2 . 插入策略表和规则组表的关联表中的数据
		if err := tx.Table(model.StrategyConfigTableName).Where("strategy_id = ?", id).Updates(&strategyInfo.StrategyConfig).Error; err != nil {
			return err
		}
		return nil //返回nil , 提交事务
	})
...
func (w wafStrategyRepo) Delete(ctx context.Context, ids []int64) (int64, error) {
	err := w.data.db.Transaction(func(tx *gorm.DB) error {
		if err := tx.Table(model.StrategyConfigTableName).Where("strategy_id in (?)", ids).Error; err != nil {
			return err
		}
		if err := tx.Table(model.StrategyTableName).Where("id in (?)", ids).Error; err != nil {
			return err
		}
		return nil
	})
